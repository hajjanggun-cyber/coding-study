...
**목적**: 코딩 용어, 프롬프트, URL, 아이디어, 실전과정, 강의정리, 손님내용, 방문예정, 물건세부, 개인자료를 체계적으로 관리하는 웹 애플리케이션
...
##### A. 탭 시스템 (10개 카테고리)
```javascript
- Coding용어: 코딩 관련 용어 정리 (기본형)
- 프롬프트: AI 프롬프트 모음 (기본형)
- 참고URL: 유용한 웹사이트 링크 (기본형)
- 아이디어: 프로젝트 아이디어 (기본형)
- 실전과정: 프로젝트 진행 과정 기록 (확장형: 접기/펼치기)
- 강의정리: 강의 내용 정리 (확장형: 접기/펼치기)
- 손님내용: 손님 관련 정보 (기본형)
- 방문예정: 방문 일정 및 내용 (기본형)
- 물건세부: 물건 상세 정보 (기본형)
- 개인자료: 개인적인 자료 정리 (확장형: 접기/펼치기)
```
...
- **순서 변경**: ⏫ 🔼 🔽 ⏬ 버튼으로 데이터 순서를 자유롭게 변경 (즉시 반응)
- **전화번호 링크**: 본문의 전화번호(예: 010-1234-5678)를 자동으로 클릭 가능한 링크로 변환
...
// 순서 변경 (DOM 직접 조작으로 즉시 반응)
function reorderItem(direction) {
  // 1. 화면에서 즉시 위치 이동 (깜빡임 없음)
  container.insertBefore(card, target);
  
  // 2. 백그라운드 서버 동기화
  fetch(API_URL, { action: 'reorder', ... });
}
```
...
| `move` | `?action=move&ids=1,2&sheetName=Coding&targetSheet=Private` | 항목 이동 (호환되는 폼끼리만 가능) |
| `reorder` | `?action=reorder&id=1&direction=up&sheetName=Coding` | 항목 순서 변경 (top, up, down, bottom) |
...
#### 시트 구조
각 시트(Coding, Prompt, URL, Ideas, WorkProcess, Jungri, Customer, Visit, ItemDetail, Private)는 다음과 같은 구조:
...
### 0. 상/하단 컨트롤 바
데이터 목록의 **위쪽과 아래쪽**에 동일한 기능을 수행하는 컨트롤 바를 배치하여, 긴 목록에서도 스크롤 없이 즉시 작업을 수행할 수 있습니다. 위/아래 버튼의 상태(예: 정렬 활성화)는 실시간으로 동기화됩니다.
...
**원인**: `.card-content`가 `display: flex`로 제목과 내용을 가로로 배치하여 내용 textarea가 제한된 공간에 갇힘  
**해결**:
```css
/* 수정 모드일 때 카드 컨텐트를 세로 배치로 변경 */
...
**브라우저 캐시 이슈**: CSS 변경 후 강력 새로고침 필요 (`Ctrl + Shift + R` 또는 `Ctrl + F5`)

### 이슈 4: 데이터 저장/수정 직후 목록이 사라지거나 초기화됨
**원인**: 데이터 저장(`saveData`) 후 1.5초 뒤에 자동으로 `fetchAllData()`를 호출하는데, 이때 서버 저장이 아직 완료되지 않았거나 네트워크 지연으로 인해 빈 데이터를 가져오는 경우 발생 (Race Condition)  
**해결**:
- 저장/수정/삭제/이동 후 `fetchAllData()` 자동 호출 로직 **제거**
- 낙관적 UI 업데이트(Optimistic UI)만 유지하여 화면은 즉시 갱신하고, 서버 통신은 백그라운드에서 조용히 수행
- 오류 발생 시에만 `alert`으로 경고
...
cachedData = {
  ...
  "Jungri": [ ... ],
  "Customer": [
    { id: 1, term: "홍길동", description: "010-1234-5678, VIP" }
  ],
  "Visit": [
    { id: 1, term: "2024-03-01", description: "오후 2시 미팅" }
  ],
  "ItemDetail": [
    { id: 1, term: "삼성 노트북", description: "Galaxy Book 4 Pro, 16GB" }
  ],
  "Private": [
    { id: 1, term: "계정 정보", description: "ID: admin\nPW: ..." }
  ]
}
```
...
**작성일**: 2026-02-10  
**버전**: 1.3  
**최종 수정**: 2026-02-11 (순서 변경 기능 추가, 전화번호 자동 링크, 하단 컨트롤 바 도입, UX/UI 반응성 개선)
